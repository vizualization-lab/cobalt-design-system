...
export async function scaffoldProject(options: ResolvedNewOptions, root: string): Promise<string> {
  ...
  await copyAgentSkill(targetDir, root);
...
async function copyAgentSkill(targetDir: string, root: string): Promise<void> {
...
This starter includes the Cobalt skill for agentic workflows at \`.codex/skills/cobalt\`. Agents can use it to query Cobalt project context, component APIs, design tokens, utilities, and diagnostics through the Cobalt CLI.
