...
export async function scaffoldProject(options: ResolvedNewOptions, root: string): Promise<string> {
  ...
  await copyAgentSkills(targetDir, root, options.agentSkill ?? 'both');
  ...
  await writeProjectReadme(targetDir, {
    ...
    agentSkill: options.agentSkill ?? 'both',
  });
...
async function copyAgentSkills(
  targetDir: string,
  root: string,
  agentSkill: ResolvedNewOptions['agentSkill'],
): Promise<void> {
  if (agentSkill === 'none') {
    return;
  }
...
async function writeProjectReadme(
  targetDir: string,
  {
    ...
    cobaltSource,
    agentSkill,
  }: Pick<ResolvedNewOptions, 'template' | 'scss' | 'appShell' | 'cobaltSource' | 'agentSkill'>,
): Promise<void> {
  ...
  const localInstallNote =
    cobaltSource === 'local'
      ...
      : '';
  const agentSkillNote = buildAgentSkillReadmeNote(agentSkill);
...
function buildAgentSkillReadmeNote(agentSkill: ResolvedNewOptions['agentSkill']): string {
  if (agentSkill === 'none') {
    return '';
  }

  const paths = [
    agentSkill === 'codex' || agentSkill === 'both' ? '`.codex/skills/cobalt`' : undefined,
    agentSkill === 'claude' || agentSkill === 'both' ? '`.claude/skills/cobalt`' : undefined,
  ]
    .filter(Boolean)
    .join(' and ');
...
This starter includes the Cobalt skill for agentic workflows at ${paths}. Agents can use it to query Cobalt project context, component APIs, design tokens, utilities, and diagnostics through the Cobalt CLI.
`;
}
